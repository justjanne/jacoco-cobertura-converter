...
internal class CoverageConverterAction(
  ...
  override fun execute(task: Task) {
    ...
    fun cover2cover(reportFile: File, outputFile: File, sourceDirectories: Iterable<File>) {
      task.project.exec {
        it.commandLine("python3")
        it.args(cover2coverScript.absolutePath)
        it.args(reportFile.absolutePath)
        it.args(sourceDirectories.map(File::getAbsolutePath))
        it.standardOutput = outputFile.outputStream()
      }
    }
    ...
    fun source2filename(reportFile: File) {
      task.project.exec {
        it.commandLine("python3")
        it.args(source2filenameScript.absolutePath)
        it.args(reportFile.absolutePath)
      }
    }
